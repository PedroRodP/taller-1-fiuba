...
fn main() {
    ...
    let buscaminas = buscar_minas_adyacentes(buscaminas);
    imprimir_tablero(buscaminas);
}

fn imprimir_tablero(tablero : Vec::<Vec::<&str>>) {
    for fila in tablero {
        for columna in fila {
            print!("{}", columna)
        }
...
fn buscar_minas_adyacentes(buscaminas : Vec::<Vec::<&str>>) -> Vec::<Vec::<&str>> {
...
fn incrementar_adyacentes(buscaminas : &Vec::<Vec::<&str>>, fila : usize, columna : usize) {
    let fila_desde = max(0, fila - 1);
    let fila_hasta = min(buscaminas.len(), fila + 1);
    let columna_desde = max(0, columna - 1);
    ...
    for f in fila_desde..fila_hasta {
        for c in columna_desde..columna_hasta {
            let mut casilla = &mut *buscaminas[f][c];
            if casilla != "*" {
                if casilla == "." { //Inicializar casillas adyacentes
                    *casilla = "0";
                }
...
fn armar_buscaminas(input_bytes: &[u8]) -> Vec::<Vec::<&str>> {
    let mut buscaminas = Vec::<Vec::<&str>>::new();
    let mut fila = 0;
    let vector = Vec::<&str>::new();
    ...
    for &byte in input_bytes {
        match byte {
            ...
            42 => buscaminas[fila].push("*"), // 42 = ASCII para .
            46 => buscaminas[fila].push("."), // 46 = ASCII para *
